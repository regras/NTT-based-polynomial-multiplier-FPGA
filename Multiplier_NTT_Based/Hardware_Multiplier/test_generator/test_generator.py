...
POLY_A_HEX_TXT = open("POLY_A_HEX.txt", "w")
POLY_B_HEX_TXT = open("POLY_B_HEX.txt", "w")
...
poly_A = [randint(0,q-1) for _ in range(N)]
poly_B = [randint(0,q-1) for _ in range(N)]
...
for i in range(N):
    ...
    POLY_A_HEX_TXT.write(hex(poly_A[i]).replace("L","")[2:]+"\n")
    POLY_B_HEX_TXT.write(hex(poly_B[i]).replace("L","")[2:]+"\n")
